...
def com_think(a_map, a_ch):
    ...
    if len(m) > 1:
        ...
        for mm in m:
            #print('mm', mm)
            threads.append(Process(target = one_turn, args = (q, a_map, a_ch, mm, player_color, mm[0], mm[1], mm[2], mm[3], 0.99, i, alpha, beta, player_color, com_color, back_num, com_ban_step, king_live, gb_m2)))
            threads[i].start()
            #threads[i].join()
            i += 1
